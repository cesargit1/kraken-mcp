...
# 2. Delete all trades
r = client.table("trades").delete().neq("id", 0).execute()
print(f"Deleted trades         : {len(r.data)} rows")

# 3. Delete all agent logs
r = client.table("agent_log").delete().neq("id", 0).execute()
print(f"Deleted agent_log      : {len(r.data)} rows")

# 3. Delete all signal states (resets last-AI-run timers, signal history, and cooldowns)
r = client.table("signal_state").delete().neq("ticker", "").execute()
print(f"Deleted signal_state   : {len(r.data)} rows")

# 4. (cooldown is now stored in signal_state — no separate table)
